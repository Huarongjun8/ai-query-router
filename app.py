...
ENRICHMENT_RULES = {
    ...
    # ── COMPARE & EVALUATE (kept, expanded) ──
    "compare": {
    ...
    # ── GENERAL (catch remaining common queries) ──
    "find": {
    ...
    # ── CHINA / CROSS-BORDER (your unique niche) ──
    "cross_border": {
...
def render_optimization_card(result):
    ...
    # Escape all dynamic content to prevent HTML breakage
    ...
    if enrichment:
        enriched_items = ", ".join(html_escape(e.replace("Consider: ", "")) for e in enrichment)
        enrichment_line = (
            f'<div style="color:#a78bfa; font-size:0.78rem; margin-top:0.3rem;">'
            f'🧠 Auto-enriched: {enriched_items}'
            f'</div>'
        )
    else:
        enrichment_line = ""
...
with st.expander("ℹ️ How Auto-Routing Works"):
    ...
    1. Rule-based enrichment detects intent across 20 categories and adds qualifiers you didn't mention — runs instantly, no API call
    ...
    - **Finance**: invest, due diligence, financial modeling, fundraising
    - **Strategy**: GTM, competitive analysis, positioning, OKRs
    - **Engineering**: code, debug, architecture, system design
    - **Business**: hire, sell/BD, product/launch, legal/compliance
    - **Analysis**: data, metrics, dashboards, reporting
    - **Communications**: write, draft, memos, investor updates
    - **Cross-Border**: China/US, regulatory, localization
    - **General**: compare, find, plan, learn, travel
