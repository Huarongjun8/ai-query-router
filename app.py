...
# Runs BEFORE the LLM optimizer to inject missing context
# ──────────────────────────────────────────────

# Intent patterns: (keywords, qualifier_checks)
# Each qualifier_check is (words_to_look_for, text_to_append_if_missing)
ENRICHMENT_RULES = {
    "buy": {
        "triggers": ["buy", "purchase", "shop", "order", "get me", "where to get", "looking for"],
        "qualifiers": [
            (["budget", "price", "cost", "cheap", "expensive", "afford", "$", "dollar", "under", "max spend"],
             "Consider: budget/price range."),
            (["location", "near", "nearby", "local", "area", "city", "store", "where", "deliver", "ship"],
             "Consider: location/availability."),
            (["best", "top", "recommend", "quality", "brand", "review", "rating", "prefer"],
             "Consider: quality preferences and priorities."),
            (["when", "urgent", "asap", "soon", "timeline", "by tomorrow", "today", "deadline"],
             "Consider: timeline/urgency."),
        ]
    },
    "build": {
        "triggers": ["build", "construct", "renovate", "remodel", "install"],
        "qualifiers": [
            (["budget", "cost", "price", "afford", "expensive", "$", "dollar"],
             "Consider: budget range."),
            (["location", "area", "city", "property", "land", "where", "zone", "permit"],
             "Consider: location and local regulations."),
            (["timeline", "when", "how long", "deadline", "schedule", "month", "year"],
             "Consider: timeline."),
            (["size", "sqft", "square", "big", "small", "bedroom", "floor", "story", "scale"],
             "Consider: size/scale requirements."),
            (["material", "wood", "concrete", "steel", "quality", "style", "modern", "traditional"],
             "Consider: materials and style preferences."),
        ]
    },
    "find": {
        "triggers": ["find", "search", "look for", "locate", "discover", "where is", "where can"],
        "qualifiers": [
            (["near", "nearby", "local", "area", "city", "mile", "km", "location", "where", "close"],
             "Consider: location/proximity."),
            (["type", "kind", "category", "specific", "criteria", "requirement", "prefer"],
             "Consider: specific criteria or requirements."),
            (["budget", "price", "cost", "free", "cheap", "$"],
             "Consider: budget constraints."),
        ]
    },
    "plan": {
        "triggers": ["plan", "organize", "schedule", "arrange", "prepare", "set up", "host"],
        "qualifiers": [
            (["budget", "cost", "price", "afford", "$", "spend"],
             "Consider: budget."),
            (["people", "person", "guest", "attendee", "group", "team", "family", "friend", "how many"],
             "Consider: number of people involved."),
            (["when", "date", "time", "timeline", "deadline", "month", "week", "day", "schedule"],
             "Consider: dates/timeline."),
            (["where", "location", "venue", "place", "indoor", "outdoor", "virtual", "online"],
             "Consider: location/venue."),
        ]
    },
    "travel": {
        "triggers": ["travel", "trip", "vacation", "visit", "fly", "flight", "hotel", "itinerary", "tour"],
        "qualifiers": [
            ...
            (["people", "solo", "couple", "family", "group", "kids", "children", "friend"],
             "Consider: who is traveling (solo, couple, family, group)."),
            (["interest", "like", "enjoy", "adventure", "relax", "culture", "food", "beach", "mountain"],
             "Consider: interests and activity preferences."),
        ]
    },
    "compare": {
        "triggers": ["compare", "versus", " vs ", "difference between", "better", "which is", "which should"],
        "qualifiers": [
            (["budget", "cost", "price", "value", "afford", "$", "worth"],
             "Consider: cost/value comparison."),
            (["priority", "important", "matter", "care about", "need", "must have", "criteria"],
             "Consider: key priorities and criteria."),
            (["use case", "purpose", "for", "scenario", "situation", "need it for"],
             "Consider: intended use case."),
        ]
    },
    "cook": {
        "triggers": ["cook", "recipe", "make food", "bake", "prepare meal", "dinner", "lunch", "breakfast"],
        "qualifiers": [
            (["people", "serving", "person", "family", "guest", "how many", "portion"],
             "Consider: number of servings."),
            (["diet", "vegan", "vegetarian", "gluten", "allergy", "intolerance", "keto", "halal", "kosher", "dairy"],
             "Consider: dietary restrictions."),
            (["time", "quick", "fast", "minute", "hour", "easy", "beginner", "simple"],
             "Consider: time available and skill level."),
            (["equipment", "oven", "grill", "stove", "tool", "pan", "pot"],
             "Consider: available cooking equipment."),
        ]
    },
    "learn": {
        "triggers": ["learn", "study", "understand", "teach me", "how to", "tutorial", "course", "explain"],
        "qualifiers": [
            ...
            (["goal", "purpose", "why", "career", "job", "hobby", "project", "certification"],
             "Consider: learning goal or purpose."),
        ]
    },
    "hire": {
        "triggers": ["hire", "recruit", "find someone", "looking for a", "contractor", "freelancer", "employee"],
        "qualifiers": [
            (["budget", "salary", "rate", "cost", "pay", "afford", "$", "hourly", "annual"],
             "Consider: budget/compensation range."),
            (["location", "remote", "onsite", "hybrid", "city", "local", "where"],
             "Consider: location requirements (remote/onsite/hybrid)."),
            (["when", "start", "timeline", "urgent", "asap", "deadline", "duration"],
             "Consider: start date and engagement duration."),
            (["skill", "experience", "senior", "junior", "qualification", "certification"],
             "Consider: required skills and experience level."),
        ]
    },
    "invest": {
        "triggers": ["invest", "investment", "stock", "portfolio", "fund", "save money", "retirement", "401k"],
        "qualifiers": [
            (["amount", "budget", "how much", "$", "dollar", "capital", "savings"],
             "Consider: investment amount/capital available."),
            (["risk", "safe", "aggressive", "conservative", "volatile"],
             "Consider: risk tolerance."),
            (["timeline", "term", "long", "short", "year", "retire", "goal", "horizon"],
             "Consider: investment timeline/horizon."),
            (["goal", "purpose", "why", "retirement", "house", "education", "income"],
             "Consider: financial goals."),
        ]
    },
    "fix": {
        "triggers": ["fix", "repair", "broken", "not working", "issue", "problem with", "troubleshoot"],
        "qualifiers": [
            (["model", "version", "brand", "type", "year", "make", "which"],
             "Consider: make/model/version details."),
            (["tried", "already", "attempted", "so far", "done"],
             "Consider: what has already been tried."),
            (["error", "message", "code", "symptom", "happens when", "shows"],
             "Consider: specific error messages or symptoms."),
        ]
    },
}
...
def enrich_prompt(user_prompt):
    """
    Rule-based enrichment. Zero cost, zero latency.
    Detects intent → injects universally relevant qualifiers the user didn't mention.
    Returns (enriched_prompt, list_of_additions).
    """
    ...
    # For each matched category, check which qualifiers are missing
    for category in matched_categories:
        rules = ENRICHMENT_RULES[category]
        for check_words, addition_text in rules["qualifiers"]:
            # If the user already mentioned any of the check words, skip
...
def render_optimization_card(result):
    ...
    if enrichment:
        enrichment_line = (
            f'<div style="color:#a78bfa; font-size:0.78rem; margin-top:0.3rem;">'
            f'🧠 Auto-enriched: {", ".join(e.replace("Consider: ", "") for e in enrichment)}'
            f'</div>'
        )
    else:
        enrichment_line = ""
...
if st.button("🚀 Send Query", type="primary", use_container_width=True):
    if query or uploaded_files:
        ...
        with st.spinner("⚡ Optimizing and routing..."):
            # ──────────────────────────────────────────
            # STEP 1: Rule-based enrichment (zero cost, zero latency)
            # ──────────────────────────────────────────
            enriched_query, enrichment_additions = enrich_prompt(query)

            # ──────────────────────────────────────────
            # STEP 2: LLM optimization via Groq (fast, near-zero cost)
            # ──────────────────────────────────────────
        ...
        # ── Show optimization card BEFORE the API call ──
        if mode == "Auto (Recommended)":
            ...
            # Manual override — still show optimization card
            manual_opt_card = {
                "model_tier": opt_result.get("recommended_model_tier", "mid"),
...
with st.expander("ℹ️ How Auto-Routing Works"):
    ...
    - **🧠 Rule-Based Enrichment**: Automatically detects your intent (buy, build, plan, travel, etc.) and adds missing context like budget, location, and timeline — zero cost, zero latency
    - **⚡ Automatic Prompt Optimization**: Every query is then refined by AI for clarity and efficiency
    ...
    1. Rule-based enrichment detects intent (buy, build, find, plan, travel, compare, cook, learn, hire, invest, fix) and adds qualifiers you didn't mention — runs instantly, no API call
    2. LLM optimizer removes filler, adds specificity, and recommends the cheapest capable model — runs via Groq (~1 sec, near-zero cost)
